...
N = 25                      # Number of sections

yMin = 6E-20                # Lower sectional grid size [kg]
yMax = 9E-12                # Upper sectional grid size [kg]

CMD = 1E-6                  # Count median diameter of the log-normal size distribution [m]
sigmag = 2.5                  # Geometric standard deviation of the log-normal size distribution
...
dfdlogdexact = logNormal(d, sigmag, CMD)

plt.plot(d, dfdlogdexact, 'k', label='Exact')
...
print('Sectional normalized fraction', dfdlogd)
print('diameter', dx)
...
#plt.style.post(fig, plt.legend())

plt.savefig('logNormalSectionalGrid.pdf')
plt.savefig('logNormalSectionalGrid.png')
